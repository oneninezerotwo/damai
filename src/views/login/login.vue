<template>
    <div id="mini-login-box">
        <div id="logo-box">
            <img src="//gw.alicdn.com/tfs/TB1QfeUQAvoK1RjSZFDXXXY3pXa-300-138.png" class="logo">
        </div>
        <div id="alibaba-login-iframe" class="iframe-loaded">
            <div id="alibaba-login-iframe-loading"></div>
            <div id="login" class="width-vertical login-label-text login-view-sms damai">
                        ...
                        <form id="login-form" class="login-form">
                            <div class="fm-field fm-field-mobile">
                                            ...
                                            <div class="picker-container" style="height: 216px;">
                                                <div class="picker-inner">
                                                    <div class="picker-column">
                                                        <div class="picker-scroller" style="transform: translate3d(0px, 90px, 0px);">
                                                            <div class="picker-item picker-item-selected" style="height: 36px; line-height: 36px;">+86 中国大陆</div>
                                                            <div class="picker-item" style="height: 36px; line-height: 36px;">+852 中国香港</div>
                                                            <div class="picker-item" style="height: 36px; line-height: 36px;">+853 中国澳门</div>
                                                            <div class="picker-item" style="height: 36px; line-height: 36px;">+886 中国台湾地区</div>
                                                        </div>
                                                    </div>
                                                    <div class="picker-highlight" style="height: 36px; margin-top: -18px;"></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="input-pwa-wrap input-wrap-sms-loginid">
                                    <input type="tel" class="fm-text" id="fm-sms-login-id" tabindex="1" placeholder="请输入手机号" autocapitalize="off" v-model="value">
                                    <div class="underline">
                            ...
                            <div class="fm-field fm-field-sms">
                                ...
                                <div class="input-pwa-wrap input-wrap-smscode">
                                    <input type="tel" class="fm-text" id="fm-smscode" tabindex="2" aria-label="请输入验证码" placeholder="请输入验证码,在打印那" maxlength="6" autocomplete="off" autocapitalize="off" v-model="psw">
                                    <div class="underline">
                                        <div class="unfocused-line"></div>
                                        <div class="focused-line"></div>
                                    </div>
                                ...
                                <div class="send-btn">
                                    <button href="javascript:void(0);" class="send-btn-link" :disabled='disable' @click="timer();ranNum()">{{btntxt}}</button>
                                </div>
                            </div>
                            <div>
                                <div id="nocaptcha-smsLogin" class="nc-container tb-login" data-nc-idx="1" style="display: none;">
                                    <div id="nc_1_wrapper" class="nc_wrapper">
                                        <div id="nc_1_n1t" class="nc_scale">
                                            ...
                                            <b id="nc_1__helpbtn" class="nc_helpbtn">
                                                <span class="nc-lang-cnt" data-nc-lang="_learning">了解新功能</span>
                                            </b>
                                        </div>
                            ...
                            <div class="fm-btn">
                                <router-link :to="url" @click.native="sendcode()" class="fm-button fm-submit sms-login">登录</router-link>
                            </div>
                            <div class="login-blocks login-links">
                                <a href="javascript:void(0);" target="_self" class="password-login-link">账号密码登录</a>
                                <a href="https://ipassport.damai.cn/member/reg/enter_reg.htm?_regfrom=DAMAI&amp;tg=https%3A%2F%2Fm.damai.cn%2Fdamai%2Fselect-seat%2Fhouniao%2Findex.html%3FitemId%3D595542197209%26performId%3D210052474%26skuId%3D4135766961110%26privilegeActId%3D%26channel%3Ddamai_app%26spm%3Da2o71.project.bottom.dbuy" target="_top" class="register-link">新用户注册</a>
                            </div>
                        </form>
                    </div>
                </div>
...
export default Vue.extend({
  ...
  methods: {
    ...
    //   var now = new Date();
    //   now.setDate(now.getDate() + 7);//iDay:5天后失效， -1：立即失效
    //   document.cookie = 'uname' + '=' + this.value + ';expires=' + now + ';path=/';
    // },
    sendcode() {
      ...
      if (this.value == "") {
        ...
        now.setDate(now.getDate() + 7); //iDay:5天后失效， -1：立即失效
        document.cookie =
          "uname" + "=" + this.value + ";expires=" + now + ";path=/";
    ...
    ranNum() {
      ...
      for (var i = 0; i < 6; i++) {
        //随机数：0-9
        this.yanzheng += parseInt(Math.random() * 10); //0-9
      }
      //	console.log(html);
    ...
    timer() {
      ...
      if (reg.test(this.value)) {
        if (this.time > 0) {
          ...
          this.disable = true;
          //  console.log(this.time,this.disable)
        } else {
          this.time = 60;
          this.btntxt = "获取验证码";
...
body.damai.mobile .width-vertical,
body.damai.mobile .width-vertical .login-content {
  width: 100%;
}
.width-vertical {
  width: 100%;
...
body.damai.mobile .master-login-title {
  display: none;
}
body.damai.mobile .width-vertical,
body.damai.mobile .width-vertical .login-content {
...
body.damai.mobile #login-form .fm-field .fm-label {
  display: none;
}
...
body.damai.mobile .phone-code-select .phone-code {
  border-color: #e7e7e7;
  border-bottom: 0;
}
...
body.damai.mobile .picker-modal-container .picker-modal {
...
body.damai.mobile .picker-modal-container .picker-modal header {
  background-color: #fff;
  border-bottom: 1px solid #eee;
}
body.damai.mobile .picker-modal-container .picker-modal header .title,
body.damai.mobile .picker-modal-container .picker-modal header a {
  color: #ff1268;
  font-size: 14px;
}
body.damai.mobile .login-view-sms a {
  font-size: 12px;
  float: left;
  margin: 0;
  color: #999;
...
body.damai.mobile .picker-modal-container .picker-item,
body.damai.mobile .picker-modal-container .picker-item-selected {
  font-size: 14px;
}
...
body.damai.mobile .input-pwa-wrap .fm-text {
  border-bottom: none;
}
body.damai.mobile .fm-field .fm-text {
  border-color: #e7e7e7;
}
.login-label-text .fm-field.fm-field-mobile .fm-text {
  padding-left: 80px;
}
body.damai.mobile .fm-text {
  border: none;
...
body.damai.mobile .input-pwa-wrap .underline .unfocused-line {
  background: #e7e7e7;
}
...
body.damai.mobile .input-pwa-wrap .underline .focused-line {
  background: #ff1268;
}
...
body.damai.mobile .fm-field-sms .send-btn {
  border-radius: 20px;
  padding: 0 10px;
  margin-bottom: 8px;
  background: #fff1f6;
...
body.damai.mobile .fm-field-sms .send-btn a {
  color: #fc347b;
}
body.damai.mobile .login-view-sms a {
  font-size: 12px;
...
body.damai.mobile .nc-container {
  margin-top: 20px;
}
...
body.damai.mobile .nc-container .nc_scale .imgCaptcha .nc_scale_submit {
...
body.damai.mobile .login-view-sms .fm-submit {
  margin-top: 0;
}
body.damai.mobile .fm-button {
  font-size: 18px;
}
body.damai.mobile .fm-button {
  background-color: #ff1268;
...
body.damai.mobile .login-view-sms .password-login-link {
  width: 50%;
  text-align: left;
}
body.damai.mobile .login-view-sms a {
